...
- **买家退回状态变化结构化复核** `return_inspections`：买家申请退货后，**订单与寄卖单同步进入退货中**（`orders.status='returning'` / 寄卖单 `buyer_returning`），仓库按 ①划痕/成色 ②附件（应退 vs 实到）③吊牌 ④防拆扣 四项逐项核对，并**必录开箱视频与仓库经办人**。**复核门禁**：未完成买家退货申请、或未提供有效开箱视频（含空白）时仓库提交复核返回 4xx（409/422），事务回滚，订单、寄卖单、复核记录、争议、退款冻结均不产生任何变化；重复复核返回 409。任一项变差即 `orders.refund_frozen` 冻结退款、在同一争议单立案，仓库平台证据（开箱视频/照片）固定入案；买家在同一争议中提交解释与证据照片，**与平台证据并列保存**。**防拆扣被拆除单独记录其对退款的影响**（脱离防调换保护、从严认定）。客服在同一争议里做责任认定（买家/平台/物流/卖家/无责）与退款影响（全额/部分扣除/拒绝），争议裁决时解冻并按裁决金额入账，部分退款的扣除额回写复核单。
- **运营管理**：质检标准（N/S/A/B/C × 品类）、鉴定师绩效（鉴定量/双人率/被推翻次数）、保险保价与理赔、仓储费、直播场次、财务台账、黑名单审计。
- **卖家底价调整与撤回结算**：商品长期在售时，卖家可**申请降价**或**撤回**。操作前服务汇总市场信号——收藏数、有效议价（含买家保证金）、活动报名、已计仓储费/保险费；降价预检给出新底价/上架价、佣金预估、预估到手，确认后更新价格并通知收藏与议价买家。撤回需卖家**显式确认**：`sellerConfirmation` 必须严格等于布尔 `true` 且 `sellerConfirmationText` 为非空文本（缺失、`false`、字符串 `"true"`/`"false"`、数字、空白文本均返回 4xx，事务回滚，寄卖单/撤回记录/结算单/议价状态/资金流水/通知均不变化）。撤回时系统通知所有议价买家交易终止原因、原路**释放保证金**（`deposit_refund` 流水）、取消活动报名，生成 `seller_withdrawals`（保留卖家确认与客服说明）与 `seller_settlements`，结算单把**仓储费、保险费、退回物流费拆开列示**并汇总扣款；对保价/物流有异议则转入同一争议流程。买家可收藏商品、带保证金议价；撤回后议价状态置为 `released_for_withdraw`。
