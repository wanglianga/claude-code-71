...
- **卖家底价调整与撤回结算**：商品长期在售时，卖家可**申请降价**或**撤回**。操作前服务汇总市场信号——收藏数、有效议价（含买家保证金）、活动报名、已计仓储费/保险费；降价预检给出新底价/上架价、佣金预估、预估到手，确认后更新价格并通知收藏与议价买家。撤回需卖家显式确认；系统通知所有议价买家交易终止原因、原路**释放保证金**（`deposit_refund` 流水）、取消活动报名，生成 `seller_withdrawals`（保留卖家确认与客服说明）与 `seller_settlements`，结算单把**仓储费、保险费、退回物流费拆开列示**并汇总扣款；对保价/物流有异议则转入同一争议流程。买家可收藏商品、带保证金议价；撤回后议价状态置为 `released_for_withdraw`。
